...
def save_predicts_embeddings(output_root, data_name, backbone_name, model_names, ckpt_name, num_classes,
                            embed_file_name, predict_file_name,
                            forget_loader, forget_label, test_loader, um_or_uram='um'):
    ...
    for model_name in model_names:
        ...
        if um_or_uram == 'um':
            ...
            if backbone_name == "resnet18":
                unlearn_model = models.resnet18(pretrained=False, num_classes=num_classes)
                # if data_name == "fmnist":
            ...
            um_feat_extractor = nn.Sequential(*list(unlearn_model.children())[:last_layers_end])
...
def save_predicts_embeddings_uram_before(lip_output_path, data_name, ckpt_name, num_classes,
                                         embed_file_name, predict_file_name,
                                         forget_loader, forget_label, test_loader):
...
if __name__ == "__main__":
    ...
    for i, data in enumerate(data_list):
        ...
        forget_loader = get_loader("forget", forget_test_data_path, args.batch_size, dataset)
        test_loader = get_loader("test", forget_test_data_path, args.batch_size, dataset)

        # uram before
        if args.load_um_or_uram == 'uram' and args.load_before_or_after == 'before':
            save_predicts_embeddings_uram_before(ouput_root, data, ckpt_name, num_classes,
                                                 embedding_file_name, pred_file_name,
                                                 forget_loader, forget_label, test_loader)
        else:
            for backbone in backbones:
                save_predicts_embeddings(ouput_root, data, backbone, unlearn_model_names, ckpt_name, num_classes,
                                         embedding_file_name, pred_file_name,
                                         forget_loader, forget_label, test_loader,
